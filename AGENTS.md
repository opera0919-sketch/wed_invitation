...
## 기능
- **홈(대시보드):** 첫 화면. 결혼식 D-day, **조치 필요 하객 카드**(완료 모임 참석자인데 참석여부 미정 → 인라인 참석/불참, 내부 스크롤), **동일인 확인 카드**(공백 무시 같은 이름 하객을 묶어 같은 분/다른 분 결정 — '삼성증권·하남고' 등 그룹/기관성 문자열은 `isGroupLike`로 제외, 결정은 합치기/스킵 기록), 청첩장 전달 달성률(**신부/신랑/공동 세그먼트 바** — 하객 탭 상단바 동일), 예산 소진 현황, 다가오는 모임 요약. (PWA는 새 배포 감지 시 자동 새로고침.)
