...
## 기능
- **하객:** 이름·구분(신부/신랑/공동)·관계·전달 여부/방식/전달자·참석 여부·메모. 전달 현황·종이 청첩장 잔량·**참석 현황 요약**. 겹지인(같은 이름 다른 구분) 자동 감지 → 공동으로 합치기.
- **모임:** 날짜·장소·성격·참석자·지출(항목/금액/결제자/분류). 완료 처리 시 참석자 자동 전달완료.
- **정산:** 신부/신랑 예산 대비 지출, 하객 구분별 배분(머릿수 비율), 큰 모임 순, 하객명단 CSV 내려받기.
- **설정:** 결혼식 날짜·예산·종이 수량, 모바일 청첩장 링크/멘트, **공동 관리자 관리**, 겹지인 기록 초기화, 전체 지우기.
