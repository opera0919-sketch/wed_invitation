...
## 기능
- **홈(대시보드):** 첫 화면. 결혼식 D-day, **조치 필요 하객 카드**(완료 모임 참석자인데 참석여부 미정 → 인라인 참석/불참, 내부 스크롤), **동일인 확인 카드**(하객 이름이 '기관명 이름' 형태로 저장되는 점을 반영해 `personKey`로 기관 접두사를 떼고 **이름 부분이 같은** 하객을 묶어 같은 분/다른 분 결정 — 접두사는 공백/붙임/없음 모두 처리, 추출한 이름이 순수 기관명이면 `isGroupLike`로 제외, 합치기 시 접두사 포함 이름 유지, 결정은 합치기/스킵 기록), 청첩장 전달 달성률(**신부/신랑/공동 세그먼트 바** — 하객 탭 상단바 동일), 예산 소진 현황, 다가오는 모임 요약. (PWA는 새 배포 감지 시 자동 새로고침.)
- **하객:** 이름·구분(신부/신랑/공동)·관계·전달 여부/방식/전달자·참석 여부·메모·**축의금(gift)**. **한 줄 리스트 + 내부 스크롤**(한 화면). 참석/미정/불참 요약 타일은 **클릭 시 필터**. 필터: 구분·**전달완료만/미전달만**·그룹 보기(목록/관계별/구분별). **일괄 수정**(전달·참석·구분·삭제), **연락처 추가**(Contact Picker), **엑셀/CSV 업로드**(이름·구분·관계·참석·메모·축의금 자동 인식, CSV는 UTF-8), CSV 내려받기. (등록은 제한 없이 — 동일인 판별은 홈 '동일인 확인' 섹션에서.)
