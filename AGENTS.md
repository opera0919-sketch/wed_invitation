...
## 기능
- **홈(대시보드):** 첫 화면. 결혼식 D-day, 청첩장 전달 달성률, 예산 소진 현황, 다가오는 모임 요약(탭 이동/모임 열기).
- **하객:** 이름·구분(신부/신랑/공동)·관계·전달 여부/방식/전달자·참석 여부·메모. 전달 현황·종이 청첩장 잔량·**참석 현황 요약**. 겹지인(같은 이름 다른 구분) 자동 감지 → 공동으로 합치기. **일괄 편집**(다중 선택 → 전달완료/미전달·구분 변경·삭제), **연락처에서 추가**(Web Contact Picker API, 안드로이드 크롬 다중선택 · 미지원 기기는 안내 토스트, 전화번호는 메모에 저장). **그룹 보기**(목록/관계별/구분별, 접기), 하객명단 **CSV 내려받기**(툴바 아이콘 버튼).
- **모임:** 날짜·장소·성격·참석자·지출(항목/금액/결제자/분류). 완료 처리 시 참석자 자동 전달완료. 참석자는 **검색으로 추가**(전체 명단 노출 대신 추가된 참석자만 표시) + **그룹 추가**(구분·관계 단위), 지난 모임인데 지출이 비면 **하단 알림 배너**.
- **정산:** **전체 청첩장 전달 진척률** + **예산 사용 금액 대조**(신부/신랑 구분 없이 합산), **이름으로 참석 모임 조회**(특정인이 참석한 모임 목록). (하객 구분별 배분·큰 모임 카드·신부/신랑 비교는 제거됨.)
- **설정:** 결혼식 날짜·예산·종이 수량, 모바일 청첩장 링크/멘트 + **링크/멘트 복사 버튼**(여기서만 노출), **공동 관리자 관리**, 겹지인 기록 초기화, 전체 지우기. (텍스트 입력은 한글 IME 안전을 위해 blur 시 저장 — `LocalText`.)
