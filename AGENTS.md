...
## 기능
- **홈(대시보드):** 첫 화면. 결혼식 D-day, **조치 필요 하객 카드**(완료 모임 참석자인데 참석여부 미정 → 인라인 참석/불참), 청첩장 전달 달성률, 예산 소진 현황, 다가오는 모임 요약.
...
- **모임:** 상단 **통계 요약**(모임 횟수·참석 인원·평균 지출). 참석자는 **검색으로 추가**(추가된 참석자만 표시) + **관계 그룹 드롭다운**(구분 '모두'는 제거) + **검색 결과 없으면 새 하객 즉시 추가**. 완료 시 참석자 자동 전달완료, 지출 미입력 지난 모임 하단 배너.
- **정산:** 전체 전달 진척률 + 예산 사용 금액 대조(합산). **이름으로 참석 모임 조회** 시 그 사람에게 쓴 **1인당 금액**(Σ 모임지출/참석인원). **받은 축의금**(엑셀 업로드로 이름 매칭 갱신, 없는 이름은 신규 추가).
- **설정:** 결혼식 날짜·예산·종이 수량, 모바일 청첩장 링크/멘트 + **링크/멘트 복사 버튼**(여기서만), **공동 관리자 관리**, 겹지인 기록 초기화, 전체 지우기. (텍스트 입력은 한글 IME 안전을 위해 blur 시 저장 — `LocalText`.)
